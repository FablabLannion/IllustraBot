...
/*          #ifdef ENABLE_M7
  ...
  while(next_statement(&letter, &value, line, &char_counter)) {
  ...
  #ifdef FABLAB_MODE
    float dx,dy;
    float length_left_side,length_right_side;
	char data;
	int tmp;
	int index_l;
	int index;
	// Length motor
	dx=target[X_AXIS];
	if (dx < X_origin+length_security) dx=X_origin+length_security;
	if (dx > X_end-length_security) dx=X_end-length_security;
    dy=target[Y_AXIS];
	if (dy < Y_origin+length_security) dy=Y_origin+length_security;
	if (dy > Y_end-length_security) dy=Y_end-length_security;
	length_left_side=sqrt((dx-X_origin)*(dx-X_origin)+(dy-Y_origin)*(dy-Y_origin));
	
	if(length_left_side>1) index_l=1;
	if(length_left_side>10) index_l=2;
	if(length_left_side>100) index_l=3;
	for(index=index_l;index>0;index--)
	{
	  if(index==1) tmp=((int)length_left_side);
	  if(index==2) tmp=((int)length_left_side)>>4;
	  if(index==3) tmp=((int)length_left_side)>>8;
	  
	  switch ((char)(tmp&0x000000000F))
	  {
	  case 0x0A : serial_write('A');break;
	  case 0x0B : serial_write('B');break;
	  case 0x0C : serial_write('C');break;
	  case 0x0D : serial_write('D');break;
	  case 0x0E : serial_write('E');break;
	  case 0x0F : serial_write('F');break;
	  default : serial_write((char)(tmp&0x000000000F)|0x30); 
	  }
	}
	serial_write(0x0d);
    ...
    // Right motor
	dx=X_end-target[X_AXIS];
	if (dx < X_origin+length_security) dx=X_origin+length_security;
	if (dx > X_end-length_security) dx=X_end-length_security;
    dy=target[Y_AXIS];
	if (dy < Y_origin+length_security) dy=Y_origin+length_security;
	if (dy > Y_end-length_security) dy=Y_end-length_security;
	length_right_side=sqrt((dx-X_origin)*(dx-X_origin)+(dy-Y_origin)*(dy-Y_origin));
	
	if(length_right_side>1) index_l=1;
	if(length_right_side>10) index_l=2;
	if(length_right_side>100) index_l=3;
	for(index=index_l;index>0;index--)
	{
	  if(index==1) tmp=((int)length_right_side);
	  if(index==2) tmp=((int)length_right_side)>>4;
	  if(index==3) tmp=((int)length_right_side)>>8;
	...
	target[X_AXIS] = length_left_side;
    target[Y_AXIS] = length_right_side;	
